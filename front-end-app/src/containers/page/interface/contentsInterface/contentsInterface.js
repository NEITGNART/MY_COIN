...
export const ContentsInterface = (props) => {
  ...
  useEffect(() => {
    ...
    // When receive new transaction from socket 1
    socket.on(socketEvt.ADD_TRANSACTION, (transaction) => {
      // adding transaction to pending transaction
  ...
  useEffect(() => {
    if (myWallet.publicKey !== "") {
      ...
      // initPeerToPeer(setBalance, myWallet, setPendingTx, setLastBlockIndex, setStateMine, socket2);
    }
  }, [myWallet.publicKey]);
        ...
        <div className="content-item  item--color-submain">
          ...
          <div className="content-item__main">
            ...
            <div className={`subTab subTab--${stateMine.state}`}>
              <i
                className={`subTab__icon fa ${stateMine.classIcon} fa-2x fa-fw`}
              ></i>
...
function initPeerToPeer(
  ...
  // socketInstance.on(socketEvt.PARSE_CHAIN, (newChain) => {
  ...
  socketInstance.on(socketEvt.ADD_NODE, (mainChain) => {
  ...
  //
  socketInstance.on(
    socketEvt.START_MINING,
    async ({ index, transactions, timeStamp, prevHash }) => {
      setStateMine(enumOptionMine.MINING);
      blockchain.setMine(true);
      transactions.push(
        blockchain.addTransactionReward(
          myWallet.publicKey.substring(2, myWallet.publicKey.length)
        )
      );
      ...
      newblock.then(async (result) => {
        if (result !== null) {
          ...
          setLastBlockIndex(blockchain.getLastBlock().index);
          socketInstance.emit(socketEvt.BROADCAST_ENDMINING, {
            result: result,
          });

          await axios.post(`http://localhost:${PORT_P2P}/stop-mining`, {
            result: result,
          });
  ...
  socketInstance.on(socketEvt.END_MINING, async (newBlock) => {
